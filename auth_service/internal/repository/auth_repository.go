...
func (repoap *AuthPostgres) CreateUser(ctx context.Context, user *model.Person) *RepositoryResponse {
	...
	return &RepositoryResponse{Success: true, Errors: nil, UserId: createdUserID}
}
func (repoap *AuthPostgres) GetUser(ctx context.Context, useremail, userpassword string) *RepositoryResponse {
	...
	if err == sql.ErrNoRows {
		return &RepositoryResponse{UserId: uuid.Nil, Success: false, Errors: erro.ErrorEmailNotRegister}
	}
	if err != nil {
		return &RepositoryResponse{UserId: uuid.Nil, Success: false, Errors: err}
	}

	err = bcrypt.CompareHashAndPassword([]byte(hashpass), []byte(userpassword))
	if err != nil {

		return &RepositoryResponse{UserId: uuid.Nil, Success: false, Errors: erro.ErrorInvalidPassword}
	}

	return &RepositoryResponse{UserId: userId, Success: true, Errors: nil}
}
