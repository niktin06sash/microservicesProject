...
func (as *AuthService) Registrate(user *model.Person, ctx context.Context) *ServiceResponse {
	...
	if err != nil {
		errMap := map[string]error{"HashPass": erro.ErrorHashPass}
	...
	if !response.Success {
		errMap := map[string]error{"CreateError": response.Errors}
		return &ServiceResponse{Success: false, Errors: errMap}
	}
	...
	return &ServiceResponse{Success: true, UserId: dbData.UserId}
}
func (as *AuthService) Authenticate(user *model.Person, ctx context.Context) *ServiceResponse {
	...
	if !response.Success {
		errMap := map[string]error{"AuthenticateError": response.Errors}
		return &ServiceResponse{Success: false, Errors: errMap}
	}
